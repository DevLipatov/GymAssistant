...
//                    error if field in database is not annotated and skip if system add many wrong fields
                    ...
                    if (i < fields.length && type!= null) {
                        ...
                        builder.append(String.format(Locale.US , DatabaseInitConst.SQL_TABLE_CREATE_FIELD_TEMPLATE, value, type));
                    }

                    ptype = type;
                }
                return String.format(Locale.US , DatabaseInitConst.SQL_TABLE_CREATE_TEMPLATE, name, builder.toString());
